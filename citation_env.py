...
class CitationEnv(gym.Env):
    ...
    def __init__(self, A: np.ndarray, B: np.ndarray, *, dt: float = 0.05, max_steps: int = 10000,
        ...
        self.ayg_hist = deque(maxlen=int(3.0/self.dt)) 
        self.I_prev = 0.0
        ...
        self._build_sigma_field(xlim=sigma_config.get("xlim", (x0 - self._padx(max_steps, dt), x0 + self._xmax(max_steps, dt) + self._padx(max_steps, dt))),
            zlim=sigma_config.get("zlim", (h0 - 200.0, h0 + 200.0)),
            nx=sigma_config.get("nx", 2400),
            nz=sigma_config.get("nz", 400),
            amplitude=sigma_config.get("amplitude", 20.0),
            wavelength=sigma_config.get("wavelength", 5000.0),
            width=sigma_config.get("width", 20.0),
            sigma_max=sigma_config.get("sigma_max", 4.0),
            sigma_min=sigma_config.get("sigma_min", 0.0),
        )
        ...
        self.action_space = spaces.Box(low = -act_high, high = act_high, dtype = np.float32) #need to recheck action space shape
    ...
    # ---------- sigma field ----------
    def _build_sigma_field(self, xlim, zlim, nx, nz, amplitude, wavelength, width, sigma_max, sigma_min): 
        x = np.linspace(*xlim, nx)
        z = np.linspace(*zlim, nz)
        X, Z = np.meshgrid(x, z)

        k = 2.0 * np.pi / wavelength
        z0 = amplitude * np.sin(k * X)
        dist = np.abs(Z - z0)
        sigma = sigma_max * (1.0 - np.exp(-(dist / width) ** 2))
    ...
    def _sigma_at_point(self, xq: float, zq: float) -> float:
        ix = np.argmin(np.abs(self.X[0] - xq))
        iz = np.argmin(np.abs(self.Z[:, 0] - zq))
    ...
    def step(self, action: np.ndarray): 
        ...
        reward = (-abs(2*ay_next) - next_state[15] + next_state[14]) if self.reward_fn is None else float(self.reward_fn(next_state, self.state, dI, self.dt)) #we set our own reward function soon 
...
def register_citation_env(env_id: str = "Citation-nl", **kwargs):
    """
    Doing this for ease.
    Then we do: gym.make("Citation-nl", A=A, B=B, render_mode='human')
    """
    from gymnasium.envs.registration import register
    try:
        register(
            id=env_id,
            entry_point="citation_env:CitationEnv",
            kwargs=kwargs,
